...
class WorkflowExecutor:
    ...
    def __init__(self, workflow_data):
        ...
        # 第三 这里构建邻接表 结构如下：
        # self.adjacency = {
        #     "node-xxxx": { "in": [("edge-yyyy", "node-yyyy"), ("edge-zzzz", "node-zzzz")], "out": [] },
        #     "node-yyyy": { "in": [], "out": [("edge-yyyy", "node-xxxx")] },
        #     ...
        # }
        # 其中第一项是边的id 第二项是对应的节点的id
        ...
        for edge in edges:
            ...
            if source_id in adjacency:
                adjacency[source_id]['out'].append((edge_id, target_id))

            # "in" relationship: the node that is the target of this edge
            if target_id in adjacency:
                adjacency[target_id]['in'].append((edge_id, source_id))
    ...
    def execute_node(self, node_id):
        ...
        result = self.node_instances[node_id].run()
        next = self.node_instances[node_id].get_next()
        return result,next
    ...
    def execute(self):
        try:
            ...
            while next_node is not None:
                ...
                result,next_node = self.execute_node(cur_node)
